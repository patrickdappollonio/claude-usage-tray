...
if (!current) {
  throw new Error(
    `Unsupported platform combination: ${process.platform}/${process.arch}. ` +
      ...
      "https://github.com/patrickdappollonio/claude-usage-tray/issues/new"
  );
}
...
if (!existsSync(binaryPath)) {
  throw new Error(
    `claude-usage-tray binary not found for ${current.key}. ` +
...
child.on("error", (err) => {
  console.error(`Failed to start claude-usage-tray: ${err.message}`);
  process.exit(1);
});
